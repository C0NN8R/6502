...
struct Memory
{
    ...
    void Load(Byte ins[256], Word MEM_START)
    {
        ...
        for (u32 i = 0; i < 256; i++)
        {
            ...
            {
                Data[MEM_START + i] = ins[i];
            }
        }
    }

    void Dump(u32 start = 0x0100, u32 length = 256, string label = "", bool format = 1)
    { // Ugly code right here, do not open pls
...
struct CPU
{
    ...
    void Exec(u32 cycles, Memory &memory)
    {
        ...
        while (cycles > 0)
        {
            ...
            // Accumulator Stuff
            case 0x69:
            { // ADC - Add to Accumulator with carry (immediate - 2 cycles) - C, Z, V, N
            ...
            break;
            case 0x29:
            { // AND - Binary AND memory with Accumulator (immediate - 2 cycles) - Z, N
                Byte Value = FetchByte(cycles, memory);
                AC &= Value;
                SR[1] = AC == 0 ? 1 : 0;
                // SR[7] = {AC bit 7 on} ? 1 : 0; ??? if zero
            ...
            break;
            case 0xC9:
            { // CMP - Compare Accumulator (immediate - 2 cycles) - C, Z, N
                Byte Value = FetchByte(cycles, memory);
                SR[1] = (AC == Value) ? 1 : 0;
                SR[0] = (AC >= Value) ? 1 : 0;
                // SR[7] = {AC bit 7 on} ? 1 : 0; ??? if zero
            ...
            // Branch Stuff
            case 0x90:
            { // BCC! - Branch on carry clear (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[0] == 0)
                {
                    PC += Value;
            ...
            break;
            case 0xB0:
            { // BCS! - Branch on carry set (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[0] == 1)
                {
                    PC += Value;
            ...
            break;
            case 0xF0:
            { // BEQ! - Branch on zero set (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[1] == 1)
                {
                    PC += Value;
            ...
            break;
            case 0x30:
            { // BMI! - Branch on negative set (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[7] == 1)
                {
                    PC += Value;
            ...
            break;
            case 0xD0:
            { // BNE! - Branch on zero clear (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[1] == 0)
                {
                    PC += Value;
            ...
            break;
            case 0x10:
            { // BPL! - Branch on negative clear (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[7] == 0)
                {
                    PC += Value;
            ...
            break;
            case 0x50:
            { // BVC! - Branch on overflow clear (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[6] == 0)
                {
                    PC += Value;
            ...
            break;
            case 0x70:
            { // BVS! - Branch on overflow set (relative - 2 cycles + 1 if page crossed + 1 if branch taken)
                Byte Value = FetchByte(cycles, memory);
                if (SR[6] == 1)
                {
                    PC += Value;
            ...
            // Flag stuff
            case 0x18:
            { // CLC! - clear carry flag (implied - 2 cycles) - C
                SR[0] = 0;
                cycles--;
            ...
            break;
            case 0xD8:
            { // CLD! - clear decimal flag (implied - 2 cycles) - D
                SR[4] = 0;
                cycles--;
            ...
            break;
            case 0x58:
            { // CLI! - clear interrupt disable flag (implied - 2 cycles) - I
                SR[3] = 0;
                cycles--;
            ...
            break;
                ...
                { // LDA - Load Accumulator (2 cycles)
                    Byte value = FetchByte(cycles, memory);
                    AC = value;
                    UpdateZeroAndNegativeFlags(AC);
                    std::cout << "    ! Ins: LDA -> 0x" << std::hex << +value << std::endl;
                }
                break;

                case 0xAA:
                { // TAX - Transfer Accumulator to X (2 cycles)
                    IX = AC;
                    UpdateZeroAndNegativeFlags(IX);
                    std::cout << "    ! Ins: TAX" << std::endl;
                    Cycle(cycles);
                }
                break;
                ...
                { // JMP - Jump (3 cycles)
                    ...
                    std::cout << "    ! Ins: JMP -> 0x" << std::hex << +PC << std::endl;
                }
                break;

                case 0x48:
                { // PHA - Push Accumulator to Stack (3 cycles)
                    Push(cycles, AC, memory);
                    std::cout << "    ! Ins: PHA" << std::endl;
                    Cycle(cycles, 2);
                }
                break;
                ...
                default:
                {
                    std::cout << "    ! ERR: Unknown instruction: 0x" << std::hex << +opcode << std::endl;
                    cycles = 0;
                }
                break;
                }
...
int main()
{
    ...
    Byte Ins[] = {
        0xA9,
        0xAB, // Load Accumulator with 0xAB
        0xAA, // Transfer Accumulator to X
        0x48, // Push Accumulator
        0x69,
        0x1D, // Add 0x1D to Accumulator
        0xC9,
        0xC8, // Compare Accumulator to 0xC8
        0xF0,
        0x02, // Branch if zero set
        0x48, // Push Accumulator
        0xEA, // NOP
        0x4C,
        0xF9,
        0xFF, // Jump to 0xFFFA (exit)
        0xEF,
    };
